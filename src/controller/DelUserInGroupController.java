 ...
 */
public class DelUserInGroupController extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public DelUserInGroupController() {
		super();
		...
		if (AuthUtil.checkUser(request) == false) {
			...
			if (SVLogin.getId_sv() != idSV) {
				...
				if (idSV == objNhom.getId_admin()) {
					response.sendRedirect(request.getContextPath() + "/thanh-vien-nhom?idNhom=" + idNhom + "&msg=1");
				} else {
					tvDAO.delThanhVienNhom(idSV, idNhom);
					response.sendRedirect(request.getContextPath() + "/thanh-vien-nhom?idNhom=" + idNhom + "&msg=2");
				}
			}
		}
	}
